...
router.get('/', async (req, res) => {
  try {
    const featuredProducts = await Product.find({isFeatured: true}).limit(6); // Fetch 6 featured products
...
router.get('/visited',async (req, res) => {
  if (!req.session.visitedProducts) {
    return res.render("product/visitedproducts", { visitedProducts: [] });
  }

  const visitedProducts = await Product.find({
    _id: { $in: req.session.visitedProducts },
  });
  res.render('product/visitedproducts',{visitedProducts});
});
